...
public class RentalService {
    ...
    private Map<String, List<Item>> userRantals = new ConcurrentHashMap<>(); // 대여한 사람의 대여 물품을 저장하기 위해 사용함.
...
//    }
    ...
    // ConcurrentHashMap으로 처리
    public CompletableFuture<Void> rentItemAsync(Item item, String userName){
        return CompletableFuture.runAsync(() -> {
            if(item.rent()){
                // computeIfAbsent -> key가 없으면 새 리스트 생성
                userRantals.computeIfAbsent(userName, k -> new CopyOnWriteArrayList<>()).add(item);
...
//    }
    ...
    public void returnItemAsync(Item item, String userName){
        executor.submit(() -> {
            List<Item> rented = userRantals.get(userName);
            if(rented != null && rented.contains(item)){
                item.returnItem();
                rented.remove(item);
                System.out.println(userName + "님이 " + item.getName() + " 반납완료! 남은 재고: " + item.getStock() + ")");
            }else{
                System.out.println("반납 실패: "+ userName + "님은" + item.getName() + "을(를) 대여하지 않았습니다.");
            }
        });
